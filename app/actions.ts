...
import { syncAndCacheUserData } from "@/lib/user-sync";
...
async function revokeGitHubAuthorization(accessToken: string) {
  ...
  console.warn(
    `GitHub grant revocation returned ${grantResponse.status}:`,
    errorText,
  );
  ...
  if (!(tokenResponse.ok || tokenResponse.status === 404)) {
    console.warn(`GitHub token revocation returned ${tokenResponse.status}`);
  }
}
...
export async function prepareGitHubReconnect() {
  const session = await auth();
  if (!session?.user?.email) throw new Error("Not authenticated");
  ...
  try {
    ...
    if (account?.accessToken) {
      try {
        await revokeGitHubAuthorization(account.accessToken);
      } catch (error) {
        console.warn("GitHub revocation failed during reconnect:", error);
      }
    }
    ...
    console.error("GitHub reconnect preparation failed:", error);
    throw new Error(`Failed to prepare reconnect: ${message}`);
  }
}
...
export async function deleteUserAccount() {
  ...
  try {
    const githubAccount = await getGitHubAccountByEmail(session.user.email);

    if (githubAccount?.accessToken) {
      try {
        await revokeGitHubAuthorization(githubAccount.accessToken);
      } catch (revokeError) {
        console.warn(
          "GitHub OAuth revocation failed during account deletion:",
          revokeError,
        );
      }
    }
